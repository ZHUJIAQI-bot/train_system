...
五个页面：**运行总览 / 售票 / 旅客列表 / 车次列表 / 统计信息**。

售票页会按方向与余票过滤车次下拉（售罄的车次直接不出现），并实时预览票价和余票；
旅客列表支持按身份证筛选、点表头排序、翻页；车次列表列出全部车次与发车时间，
选定行程后还能看到每趟车的余票；统计页按车次和日期显示车厢占用与区段载客。

网页端特有、桌面版做不到的几点：

- **深色模式**：跟随系统，也可手动在「跟随系统 / 浅色 / 深色」之间循环切换
- **手机端适配**：窄屏下表格自动变成卡片式布局，输入框拉满方便点按
- **导出 / 导入存档**：存档可以下载成文件，也能从文件还原
...
```bash
node web/test/smoke.mjs      # wasm 侧业务逻辑（90 项）
node web/test/dom-smoke.mjs  # 界面层接线（56 项，需 jsdom）
```
...
旅客筛选、车次下拉的方向与余票过滤、运行总览、车次列表、主题切换、
排序分页、标签页切换、语言切换、i18n 渲染。
